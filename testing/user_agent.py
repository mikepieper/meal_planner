...
def check_goal_achievement(state: UserState, config: UserAgentConfig) -> bool:
...
def check_conversation_end(state: UserState, config: UserAgentConfig) -> tuple[bool, Optional[str]]:
...
def create_simple_user_agent(config: UserAgentConfig = None) -> Any:
    ...
    def process_user_turn(state: UserState) -> Dict[str, Any]:
        """Generate user response and check if conversation should end - combined logic."""
        ...
        if scenario.specific_requirements:
            req_text = ", ".join([f"{k}: {v}" for k, v in scenario.specific_requirements.items() 
                                if k not in ['task', 'success_indicators', 'opening_message']])
            if req_text:
...
Generate your next response as {persona.name}:"""
        ...
        # Update state with new message and incremented turn
        new_messages = state.messages + [HumanMessage(content=response_content)]
        new_turn_count = state.turn_count + 1
        
        # Check goal achievement (using updated state)
        ...
        goal_achieved = check_goal_achievement(updated_state, config)
        
        # Check if conversation should end
        updated_state.goal_achieved = goal_achieved
        should_end, end_reason = check_conversation_end(updated_state, config)
        
        # Return all updates in one go
        return {
            "messages": new_messages,
            "turn_count": new_turn_count,
    ...
    # Ultra-simple graph with just 1 node
    graph = StateGraph(UserState)
    graph.add_node("process_turn", process_user_turn)
...
def initialize_simple_user_state(scenario: TestScenario) -> UserState:
